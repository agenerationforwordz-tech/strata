...
def _get_db():
    ...
    Uses the same env vars as db_pg.py for consistency."""
    conn = psycopg2.connect(
        host=os.environ.get("STRATA_PG_HOST", "localhost"),
        port=int(os.environ.get("STRATA_PG_PORT", "5432")),
        dbname=os.environ.get("STRATA_PG_DB", "strata_db"),
        user=os.environ.get("STRATA_PG_USER", "strata"),
        password=os.environ.get("STRATA_PG_PASSWORD", "strata_brain_2026"),
    )
...
def init_auth_tables():
    ...
    Schema created by strata_pg_schema.sql."""
    conn = _get_db()
    try:
        cur = conn.cursor()
